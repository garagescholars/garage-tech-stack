...
exports.gsHiringVideoReminder = exports.gsHiringWeeklyDigest = exports.gsProcessInterviewScore = exports.gsCalBookingWebhook = exports.gsProcessVideoCompletion = exports.gsScoreHiringApplication = exports.gsVerifyVideoAccess = void 0;
...
function videoReminderEmail(name, videoLink) {
    const safe = escapeHtml(name);
...
exports.gsHiringWeeklyDigest = (0, scheduler_1.onSchedule)({
    ...
    try {
        ...
        const videoDropoffs = applicants.filter((a) => a.videoInvitedAt && !a.videoCompletedAt && a.status === "rejected").length;
        const videoDropoffRate = passedApp > 0 ? Math.round((videoDropoffs / passedApp) * 100) : 0;
        // Time-to-hire (avg days for hired candidates)
        const hiredWithTimes = applicants.filter((a) => a.decision === "hire" && a.appliedAt && a.decisionAt);
        const avgDaysToHire = hiredWithTimes.length > 0
            ? Math.round(hiredWithTimes.reduce((sum, a) => sum + (a.decisionAt.toMillis() - a.appliedAt.toMillis()) / (1000 * 60 * 60 * 24), 0) / hiredWithTimes.length)
            : null;
        // Source breakdown
        const bySource = {};
        const bySourceWeek = {};
 ...
 */
exports.gsHiringVideoReminder = (0, scheduler_1.onSchedule)({
    schedule: "0 */6 * * *",
    timeZone: "America/Denver",
    memory: "256MiB",
    timeoutSeconds: 60,
    ...
    try {
        ...
        for (const doc of snapshot.docs) {
            ...
            if (elapsed >= NINETY_SIX_HOURS) {
                ...
                await sendHiringEmail([applicant.email], "Update on Your Garage Scholars Application", rejectionEmail(firstName(applicant.name), "video"));
                await sendHiringEmail(FOUNDER_EMAILS, `[GS Hiring] ${escapeHtml(applicant.name)} auto-rejected (video timeout 96h)`, emailWrapper("Video Timeout", escapeHtml(applicant.name), `
              ...
              <p style="color:#475569;font-size:13px;">Invited at: ${new Date(invitedAtMs).toLocaleString("en-US", { timeZone: "America/Denver" })}</p>`));
                ...
                const tokenDoc = await db
                    .collection(gs_constants_1.GS_COLLECTIONS.HIRING_VIDEO_TOKENS)
                    .doc(doc.id)
                    .get();
                ...
                await sendHiringEmail([applicant.email], "Reminder — Complete Your Garage Scholars Video Screen", videoReminderEmail(firstName(applicant.name), videoLink));
