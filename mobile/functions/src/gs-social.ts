...
const DONATION_THEME: ColorTheme = {
...
function isAngleInSeason(angle: string): boolean {
  const match = angle.match(/\[SEASONAL:\s*(\w+)-(\w+)\]/);
  if (!match) return true; // Non-seasonal = always available
...
async function getNextTypedCaptionAngle(
  configDocName: string,
  angles: string[],
): Promise<{ angle: string; lastCaption: string }> {
  const configRef = db.collection(GS_COLLECTIONS.PLATFORM_CONFIG).doc(configDocName);
  try {
...
async function saveTypedLastCaption(configDocName: string, caption: string) {
  try {
    await db.collection(GS_COLLECTIONS.PLATFORM_CONFIG).doc(configDocName).set(
...
async function createResaleComposite(
  photoBuffers: Buffer[],
  itemName: string,
  theme: ColorTheme,
): Promise<Buffer> {
  ...
  const labelSvgs = labels.map((label) =>
    Buffer.from(`<svg width="${CELL_W}" height="${labelHeight}">
...
async function createDonationComposite(
  photoBuffers: Buffer[],
  theme: ColorTheme,
): Promise<Buffer> {
  const CANVAS = 1080;
...
async function createGymComposite(
  photoBuffers: Buffer[],
  ...
  const heroImg = await sharp(heroBuffer).resize(INNER, HERO_HEIGHT, { fit: "cover", position: "centre" }).jpeg({ quality: 92 }).toBuffer();
  ...
  for (let i = 1; i <= 2; i++) {
    const buf = photoBuffers[i] || photoBuffers[0];
    const enhanced = await enhancePhoto(buf);
    thumbs.push(await sharp(enhanced).resize(THUMB_W, THUMB_HEIGHT, { fit: "cover", position: "centre" }).jpeg({ quality: 92 }).toBuffer());
  }
...
async function generateTypedCaption(
  contentType: string,
  jobTitle: string,
  address: string,
  packageTier: string,
  ...
  async () => {
    ...
    for (const doc of allDocs) {
      ...
      try {
        ...
        const contentType = (item.contentType || "before_after") as string;
        let compositeBuffer: Buffer;
        let caption: string;

        if (contentType === "resale" || contentType === "donation" || contentType === "gym_install") {
          // ─── NEW CONTENT TYPES: resale, donation, gym_install ───
          const itemPhotos = (item.itemPhotos || []) as string[];
          if (itemPhotos.length === 0) throw new Error(`No item photos found for ${contentType} post`);

          // Download all item photos
          const photoBuffers: Buffer[] = [];
          for (const url of itemPhotos) {
            const resp = await fetch(url);
            if (!resp.ok) throw new Error(`Failed to download photo: ${url}`);
          ...
          if (!firstCheck.valid) throw new Error(firstCheck.reason!);
          ...
          caption = await generateTypedCaption(
            contentType, item.jobTitle, item.address, item.packageTier, itemName,
          );
        } else {
        ...
        if (!captionCheck.valid) {
          ...
          if (contentType === "before_after" || !item.contentType) {
            ...
            caption = await generateTypedCaption(
              contentType, item.jobTitle, item.address, item.packageTier, item.itemName,
            );
          }
