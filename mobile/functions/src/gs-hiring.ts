...
function emailWrapper(title: string, subtitle: string, bodyHtml: string): string {
  return `<!DOCTYPE html>
...
function rejectionEmail(name: string, stage: "application" | "video" | "final"): string {
...
function videoInviteEmail(name: string, videoLink: string): string {
  const safe = escapeHtml(name);
...
function zoomInviteEmail(name: string, calLink: string): string {
  const safe = escapeHtml(name);
...
async function founderDossierEmail(applicant: HiringApplicant, zoomTime: string): Promise<string> {
...
function offerEmail(name: string): string {
  const safe = escapeHtml(name);
...
function decisionSummaryEmail(
  ...
  const decisionColor =
    decision === "HIRE" ? "#166534" : decision === "REJECT" ? "#991b1b" : "#92400e";
  const decisionBg =
    decision === "HIRE" ? "#f0fdf4" : decision === "REJECT" ? "#fef2f2" : "#fffbeb";
...
async function founderReviewEmail(applicant: HiringApplicant, finalScore: number): Promise<string> {
  ...
  return emailWrapper(
    `Review Needed: ${escapeHtml(applicant.name)}`,
    `Score: ${finalScore}/100 — needs your call`,
    `
    <p style="color:#475569;font-size:15px;line-height:1.6;margin:0 0 16px;">
...
function validateApplicantData(data: HiringApplicant): void {
  // Field length limits
  const textFields = [
...
async function sendHiringEmail(to: string[], subject: string, html: string): Promise<void> {
...
async function scoreVideosWithGemini(
  storagePaths: string[],
  applicantName: string,
): Promise<VideoScoringResult> {
  ...
  return parseClaudeJson<VideoScoringResult>(responseText);
}
 ...
 */
export const gsVerifyVideoAccess = onRequest(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const tokenDoc = await db.collection(GS_COLLECTIONS.HIRING_VIDEO_TOKENS).doc(id).get();
      ...
      const applicantDoc = await db.collection(GS_COLLECTIONS.HIRING_APPLICANTS).doc(id).get();
      if (!applicantDoc.exists) {
        res.status(404).json({ valid: false, error: "not_found" });
        return;
      }

      const data = applicantDoc.data() as HiringApplicant;
 ...
 */
export const gsScoreHiringApplication = onDocumentCreated(
  {
    document: `${GS_COLLECTIONS.HIRING_APPLICANTS}/{appId}`,
    memory: "512MiB",
    timeoutSeconds: 120,
    secrets: ["ANTHROPIC_API_KEY"],
  },
  async (event) => {
    const appData = event.data?.data() as HiringApplicant | undefined;
    if (!appData) return;

    const appId = event.params.appId;
    const docRef = db.collection(GS_COLLECTIONS.HIRING_APPLICANTS).doc(appId);
    ...
    try {
      ...
      if (!dupeCheck.empty) {
        ...
        await docRef.update({ status: "rejected", decision: "reject", decisionAt: FieldValue.serverTimestamp() });
        return;
      }
...
<applicant_answer question="6_interest">${appData.q6_interest}</applicant_answer>${resumeNote}`;
      ...
      const scores = parseClaudeJson<AppScoringResult>(rawResponse);
      ...
      if (scores.pass) {
        ...
        await db.collection(GS_COLLECTIONS.HIRING_VIDEO_TOKENS).doc(appId).set({
          token: videoToken,
          createdAt: FieldValue.serverTimestamp(),
        });
        await docRef.update({
        ...
        await sendHiringEmail(
          [appData.email],
          "Next Step — Garage Scholars Video Screen (5 min)",
          videoInviteEmail(firstName(appData.name), videoLink),
        );

        // Notify founders
        await sendHiringEmail(
          FOUNDER_EMAILS,
          `[GS Hiring] ${safeName} PASSED app screen (${scores.composite_score}/100)`,
          emailWrapper("Applicant Passed", `${safeName} — Score: ${scores.composite_score}/100`, `
        ...
        await sendHiringEmail(
          ...
          rejectionEmail(firstName(appData.name), "application"),
        );

        // Notify founders
        await sendHiringEmail(
          FOUNDER_EMAILS,
          `[GS Hiring] ${safeName} REJECTED at app screen (${scores.composite_score}/100)`,
          emailWrapper("Applicant Rejected", `${safeName} — Score: ${scores.composite_score}/100`, `
            <p style="color:#475569;font-size:14px;line-height:1.6;">${escapeHtml(scores.summary)}</p>
      ...
      await sendHiringEmail(
        FOUNDER_EMAILS,
        `[GS Hiring ERROR] Failed to score: ${safeName}`,
        emailWrapper("Scoring Error", appId, `
 ...
 */
export const gsProcessVideoCompletion = onDocumentCreated(
  {
    document: `${GS_COLLECTIONS.HIRING_VIDEO_COMPLETIONS}/{completionId}`,
    memory: "1GiB",
    timeoutSeconds: 300,
    secrets: ["GEMINI_API_KEY"],
  },
  async (event) => {
    const completion = event.data?.data() as VideoCompletionEvent | undefined;
    ...
    if (!storagePaths.every((p: string) => typeof p === "string" && p.startsWith(`hiring-videos/${applicantId}/`))) {
      console.error(`[Hiring] Video completion invalid storage paths for ${applicantId}`);
      return;
    }

    const applicantRef = db.collection(GS_COLLECTIONS.HIRING_APPLICANTS).doc(applicantId);
    ...
    const applicant = applicantSnap.data() as HiringApplicant;
    ...
    try {
      ...
      if (scores.pass) {
        ...
        await sendHiringEmail(
          [applicant.email],
          "Garage Scholars — Let's Talk (15 min Zoom)",
          zoomInviteEmail(firstName(applicant.name), CAL_LINK),
        );

        // Notify founders
        await sendHiringEmail(
          FOUNDER_EMAILS,
          `[GS Hiring] ${escapeHtml(applicant.name)} PASSED video screen (${scores.composite_score}/100)`,
          emailWrapper("Video Screen Passed", `${escapeHtml(applicant.name)} — Video: ${scores.composite_score}/100`, `
            <p style="color:#475569;font-size:14px;line-height:1.6;">${escapeHtml(scores.summary)}</p>
        ...
        await sendHiringEmail(
          FOUNDER_EMAILS,
          `[GS Hiring] ${escapeHtml(applicant.name)} REJECTED at video screen (${scores.composite_score}/100)`,
          emailWrapper("Video Screen Failed", `${escapeHtml(applicant.name)} — Video: ${scores.composite_score}/100`, `
            <p style="color:#475569;font-size:14px;line-height:1.6;">${escapeHtml(scores.summary)}</p>
            ${scores.red_flags.length ? `<p style="color:#991b1b;font-size:13px;">Red flags: ${scores.red_flags.map(f => escapeHtml(f)).join(", ")}</p>` : ""}
      ...
      await sendHiringEmail(
        FOUNDER_EMAILS,
        `[GS Hiring ERROR] Video processing failed: ${escapeHtml(applicant.name)}`,
        emailWrapper("Video Processing Error", applicantId, `
 ...
 */
export const gsCalBookingWebhook = onRequest(
  { memory: "256MiB", timeoutSeconds: 30, secrets: ["CAL_WEBHOOK_SECRET"] },
  async (req, res) => {
    if (req.method !== "POST") {
      res.status(405).send("Method not allowed");
      return;
    ...
    try {
      ...
      await doc.ref.update({
        ...
        zoomScheduledAt: FieldValue.serverTimestamp(),
      ...
      await sendHiringEmail(
        FOUNDER_EMAILS,
        `[GS INTERVIEW] ${escapeHtml(applicant.name)} — ${escapeHtml(zoomTime)}`,
        await founderDossierEmail(applicant, zoomTime),
      );
 ...
 */
export const gsProcessInterviewScore = onDocumentCreated(
  {
    document: `${GS_COLLECTIONS.HIRING_INTERVIEW_SCORES}/{scoreId}`,
    memory: "256MiB",
    timeoutSeconds: 60,
  },
  async (event) => {
    const scoreEvent = event.data?.data() as InterviewScoreEvent | undefined;
    ...
    try {
      ...
      const appPoints = applicant.appScores
        ? (applicant.appScores.composite_score / 100) * (DECISION_WEIGHTS.APP_SCORE * 100)
        : 0;
      const videoPoints = applicant.videoScores
        ? (applicant.videoScores.composite_score / 100) * (DECISION_WEIGHTS.VIDEO_SCORE * 100)
        : 0;
      const zoomPoints = (zoomAvg / 5) * (DECISION_WEIGHTS.ZOOM_SCORE * 100);

      const finalComposite = Math.round(appPoints + videoPoints + zoomPoints);

      // Apply decision logic
      let decision: "hire" | "reject" | "review";
      let decisionLabel: "HIRE" | "REJECT" | "REVIEW";
      ...
      } else if (finalComposite >= DECISION_THRESHOLDS.HIRE) {
        decision = "hire";
        decisionLabel = "HIRE";
      } else if (finalComposite >= DECISION_THRESHOLDS.REVIEW_MIN) {
      ...
      if (decision === "hire") {
        ...
        await sendHiringEmail(
          [applicant.email],
          "Welcome to Garage Scholars",
          offerEmail(firstName(applicant.name)),
        );
        ...
        await sendHiringEmail(
          [applicant.email],
          "Update on Your Garage Scholars Application",
          rejectionEmail(firstName(applicant.name), "final"),
        );
      }
      ...
      await sendHiringEmail(
        FOUNDER_EMAILS,
        `[GS DECISION] ${escapeHtml(applicant.name)} — ${decisionLabel} (${finalComposite}/100)`,
        founderEmailHtml,
      );
      ...
      await sendHiringEmail(
        FOUNDER_EMAILS,
        `[GS Hiring ERROR] Decision engine failed: ${escapeHtml(applicant.name)}`,
        emailWrapper("Decision Error", applicantId, `
 ...
 */
export const gsHiringWeeklyDigest = onSchedule(
  {
  ...
  async () => {
    ...
    try {
      const snapshot = await db.collection(GS_COLLECTIONS.HIRING_APPLICANTS).get();
      const applicants = snapshot.docs.map((d) => d.data() as HiringApplicant);
      ...
      const newThisWeek = applicants.filter(
        (a) => a.appliedAt && a.appliedAt.toMillis() > weekAgo.toMillis(),
      ).length;
      const hiredThisWeek = applicants.filter(
        (a) => a.decision === "hire" && a.decisionAt && a.decisionAt.toMillis() > weekAgo.toMillis(),
      ).length;
      const rejectedThisWeek = applicants.filter(
