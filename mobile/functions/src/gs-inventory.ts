...
export const gsAnalyzeItem = onCall(
  {
    cors: true,
    timeoutSeconds: 60,
    memory: "512MiB",
  ...
  async (request) => {
    const { photoUrls, itemType } = request.data as {
      photoUrls: string[];
...
{"name":"...","description":"...","condition":"...","category":"..."}`;
      ...
      throw new HttpsError("internal", `Item analysis failed: ${err.message}`);
    }
  }
);
...
export const gsOnItemConfirmed = onDocumentUpdated(
  `${GS_COLLECTIONS.RESALE_DONATION_ITEMS}/{itemId}`,
  async (event) => {
    ...
    if (before.status === "worker_confirmed" || after.status !== "worker_confirmed") return;

    const itemType = after.type as "resale" | "donation";
    const jobId = after.jobId as string;
    ...
    const photos = after.photos as Record<string, string>;
    const confirmed = after.workerConfirmed as { name: string; description: string };
...
export const gsOnGymPhotosUploaded = onDocumentCreated(
  `${GS_COLLECTIONS.GYM_INSTALL_PHOTOS}/{docId}`,
  async (event) => {
    ...
    const equipment = data.equipmentInstalled as string[];
...
export const gsOnDonationReceiptUploaded = onDocumentCreated(
  `${GS_COLLECTIONS.DONATION_RECEIPTS}/{receiptId}`,
  async (event) => {
    const data = event.data?.data();
    if (!data) return;

    const jobId = data.jobId as string;
    const donationCenter = data.donationCenter as string;
    const receiptPhotoUrl = data.receiptPhotoUrl as string;
    ...
    const job = jobSnap.data()!;
    const customerEmail = job.clientEmail as string;
    const customerName = job.clientName || job.customerName || "Valued Customer";

    // Load donated items
    const itemNames: string[] = [];
    for (const itemId of itemIds) {
      const itemSnap = await db.collection(GS_COLLECTIONS.RESALE_DONATION_ITEMS).doc(itemId).get();
      if (itemSnap.exists) {
    ...
    try {
      await sendEmail(
        recipients,
        `Donation Receipt — ${donationCenter} | ${job.title || "Garage Transformation"}`,
        emailBody
      );
