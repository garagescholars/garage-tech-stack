...
export {
  gsMultiNotify,
  gsOnEscalationCreated,
  gsJobPrepReminders,
} from "./gs-notifications";
...
const buildSopUserMessage = (job: FirebaseFirestore.DocumentData, hasImages: boolean, adminNotes?: string) => {
  ...
  const gymEquipment: Array<{id: string; qty: number; customName?: string}> = job.productSelections?.gymEquipment || [];
  if (gymEquipment.length > 0) {
    const gymNotes: string[] = [];
    // Inline catalog lookup — matches IDs from the GYM_EQUIPMENT_CATALOG in productCatalog.ts
    const GYM_CATALOG: Record<string, {name: string; brand: string; dims: string; assemblyTime: string; crewSize: number; tools: string[]; manualUrl: string; notes: string}> = {
    ...
    for (const item of gymEquipment) {
      ...
      if (cat) {
        gymNotes.push(
          `- ${item.qty}x ${cat.name} (${cat.brand}) — ${cat.dims}, ${cat.assemblyTime} assembly, ${cat.crewSize}+ crew. Tools: ${cat.tools.join(", ")}. Manual: ${cat.manualUrl || "check included manual"}${cat.notes ? `. ${cat.notes}` : ""}`
        );
...
// ── Helper: fetch manufacturer PDF manuals for Claude document context ──
 ...
 */
async function fetchManualPdfs(
  productSelections: Record<string, any> | undefined,
  maxDocs = 3,
): Promise<DocBlock[]> {
  if (!productSelections) return [];

  const { lookupEquipment } = await import("./gs-catalog");

  // Collect unique manual URLs from all equipment
  const manualUrls: Array<{ id: string; name: string; url: string }> = [];
  const seenUrls = new Set<string>();

  const addUrl = (id: string, name: string, url: string) => {
    if (!url || url === "N/A" || seenUrls.has(url) || !url.endsWith(".pdf")) return;
    seenUrls.add(url);
  ...
  if (productSelections.gymEquipment) {
    for (const item of productSelections.gymEquipment) {
      ...
      if (entry) addUrl(item.id, entry.name, entry.manualUrl);
    }
  }
  // Bold Series
  if (productSelections.boldSeriesId) {
    const entry = lookupEquipment(productSelections.boldSeriesId);
    if (entry) addUrl(productSelections.boldSeriesId, entry.name, entry.manualUrl);
  }
  // Shelving
  ...
  if (productSelections.flooringId && productSelections.flooringId !== "none") {
    ...
    if (entry) addUrl(productSelections.flooringId, entry.name, entry.manualUrl);
  }
...
// ═══════════════════════════════════════════════════════════════
export const gsGenerateAssemblyGuide = onCall(
  ...
  async (request) => {
    ...
    const { equipmentId, manualUrl, productName } = request.data as {
      equipmentId?: string;
      manualUrl?: string;
      productName?: string;
    };
    ...
    if (!equipmentId && !manualUrl) {
      throw new HttpsError("invalid-argument", "Provide equipmentId or manualUrl.");
    }
    ...
    if (!resolvedUrl) {
      throw new HttpsError("invalid-argument", `No manual URL found for ${resolvedName}. Provide a manualUrl.`);
    }
...
TONE: Direct, friendly, patient. Like a patient older coworker showing you the ropes.`;
    ...
    try {
      ...
      const textBlock = response.content.find((b: any) => b.type === "text");
      const guide = textBlock ? (textBlock as any).text : "";
      ...
      if ((error as any).code) throw error; // re-throw HttpsError
      console.error("Assembly guide generation failed:", error);
      throw new HttpsError("internal", `Failed to generate guide: ${(error as Error).message}`);
    }
  },
);
