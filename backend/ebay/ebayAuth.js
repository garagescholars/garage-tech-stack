...
console.log("EBAY_REDIRECT_URI from env =", EBAY_REDIRECT_URI);
...
const logAxiosError = (error, context) => {
    ...
    console.error('❌ eBay auth request failed:', { ...context, status, details });
};
...
const refreshAccessToken = async (db) => {
    ...
    if (!stored || !stored.refreshToken) {
        throw new Error('Missing eBay refresh token in Firestore');
    }
